...
func Test_deploymentReconciler_Reconcile(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				clientSetFn: func(t *testing.T) ecosystemClientSet {
					...
					deploymentMock.EXPECT().Get(testCtx, testComponentName, metav1.GetOptions{}).
		...
		},
		{
			...
			wantErr: func(t assert.TestingT, err error, i ...interface{}) bool {
				return assert.ErrorIs(t, err, assert.AnError, i) &&
					assert.ErrorContains(t, err, fmt.Sprintf("failed to get deployment \"%s/%s\"", testNamespace, testComponentName), i)
			},
		},
		{
		...
		},
		{
			...
			wantErr: func(t assert.TestingT, err error, i ...interface{}) bool {
				return assert.ErrorIs(t, err, assert.AnError, i) &&
					assert.ErrorContains(t, err, fmt.Sprintf("failed to update component health for deployment \"%s/%s\"", testNamespace, testComponentName), i)
			},
		},
		{
